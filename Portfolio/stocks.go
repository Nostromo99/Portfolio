...
func add(addition string) {
	file, err := os.OpenFile("portfolio/tickers.txt", os.O_RDWR, 0755)
	if err != nil {
...
func remove(removal string) {
	file, _ := ioutil.ReadFile("portfolio/tickers.txt")
	...
	ioutil.WriteFile("portfolio/tickers.txt", []byte(replacement), 0644)
